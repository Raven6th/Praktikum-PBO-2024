...
interface ChessPersonInterface {
    String getName();
    ...
    String getPreferredOpenings();
    void setPreferredOpenings(String preferredOpenings);
    void displayRole();
}


class ChessPlayer implements ChessPersonInterface {
    private String name;
    private int age;
    private int rating;
    ...
    public void displayRole() {
        System.out.println("Role: Casual");
    }
...
public class Main {
    ...
    private static TopChessPlayers topChessPlayers = new TopChessPlayers();
    private static ArrayList<ChessPersonInterface> interfacePlayers = new ArrayList<>();
    ...
    private static void addPlayer() {
        ...
        switch (playerType) {
            case 1:
                addPlayerInterface(new ChessPlayer(name, age, rating, preferredOpenings));
                break;
            case 2:
    ...
    private static void addPlayerInterface(ChessPersonInterface player) {
        interfacePlayers.add(player);
        System.out.println("Profil telah ditambahkan!");
    }

    private static void viewAllPlayers() {
        if (players.isEmpty() && interfacePlayers.isEmpty()) {
        ...
        for (ChessPersonInterface interfacePlayers : interfacePlayers) {
            interfacePlayers.displayRole();
            System.out.println(interfacePlayers);
        }
    ...
    private static void updatePlayer() {
        System.out.println("\nMasukkan jenis profil yang ingin diedit (1: Kasual, 2: Profesional, 3: Coach): ");
        int playerType = Integer.parseInt(scanner.nextLine());
        ...
        switch(playerType){
            case 1:
                for (ChessPersonInterface interfacePlayers : interfacePlayers) {
                    if (interfacePlayers.getName().equalsIgnoreCase(name)) {
                        System.out.println("Masukkan detail:");
                        System.out.print("Nama: ");
                        String newName = scanner.nextLine();
                        interfacePlayers.setName(newName);

                        System.out.print("Umur: ");
                        int age = Integer.parseInt(scanner.nextLine());
                        interfacePlayers.setAge(age);

                        System.out.print("Rating: ");
                        int rating = Integer.parseInt(scanner.nextLine());
                        interfacePlayers.setRating(rating);

                        System.out.print("Pembuka Favorit: ");
                        String preferredOpenings = scanner.nextLine();
                        interfacePlayers.setPreferredOpenings(preferredOpenings);
    ...
    private static void deletePlayer() {
        ...
        switch(playerType){
            case 1:
                removed = interfacePlayers.removeIf(interfacePlayers -> interfacePlayers.getName().equalsIgnoreCase(name));
                ...
                break;
            case 2:
                removed = players.removeIf(player -> player.getName().equalsIgnoreCase(name));
